...
as a set of goals to be built, and will build them.  Each goal may be the
target of an ordinary Make rule, or one of the following Minion constructs:
...
name of the file generated by the rule.  Not all rules generate output
files; those that do not are called *phony* rules, and the target name
simply serves to identify the rule.

We use the term *target ID* to mean one of three things:

#. A target name of an ordinary Make rule defined in your Makefile.
#. A Minion *instance*.
#. A source file name.

We use the term *target list* to mean a space-delimited list of target IDs
and/or indirections (each of which will be expanded to a list of target
IDs.)

## Instances

A "instance" is an expression that takes the form: `CLASS(ARGS)`.

For example, `CC(hello.c)` is an instance denotes an object file that
...
`.in` variable, if defined, identifies all the target IDs that the alias
will cause to be built.  It can contain indirections or target IDs.  The
value of the `.command` variable, if defined, is a shell command to be
executed when the alias is named as a goal.  If both are defined, the
command will be executed after all the target IDs named in `.in` are buit.

## Indirections

An "indirection" is an expression that names a variable.  Indirections may
appear in [target lists](#target-ids-and-lists) or on the command line.  In
these contexts, an "expansion" step is performed, in which indirections are
replaced with the target IDs they reference.  The result of expansion is
zero or more target IDs (instances or ordinary target names).
...
are recursively processed during expansion.
...
optional `NAME:` prefix.  Arguments without such a prefix are called
"unnamed arguments".

Remember that argument lists, being part of an instance name, may not
contain whitespace.
...
The value of `in` is a [target list](#target-ids-and-lists), and it defaults
to `$(_args)` (all unnamed arguments to the instance).

Typically, an instance can be thought of as a function that accepts one or
more input files as arguments, but there are exceptions to this rule.  In
those cases, the user may override `{in}` for a class or instance.

Targets listed in `{in}` will be treated as pre-requisites of the instance's
rule.  The output files corresponding to the targets named in `{in}` are
...
most derived classes define only `{outExt}` and leave `{out}` and the other
related properties unchanged.

Note that Builder's definition of `{out}` is designed to ensure that
different instance names generate different output file paths, in order to
avoid conflicts between instances.  Its directory components incorporate the
class name information about all the arguments.  When overriding `{outDir}`,
`{outName}`, or `{out}` itself, keep in mind that no two instances can share
the same output file name.  Also, be aware that Minion's `make clean` simply
executes `rm -rf $(OUTDIR)`, so it will not remove any output files whose
`{out}` properties have placed them outside of `$(OUTDIR)`.
...
`LinkC(hello.c)`.  Each subclass of Builder can define its own inference
rules by overriding `{inferClasses}`.  It consists of a list of entries of
the form `CLASS.EXT`, each indicating that `CLASS` should be applied to a
input file ending in `.EXT`.

Note that when an intermediate file is inferred, `{^}` will contain the file
name of the *direct* dependency (the intermediate file), while the
coresponding entry in `{in}` will refer to the [target
ID](#target-ids-and-lists) of the indirect dependency.
