...
Within your makefile, files that are inputs to (prerequisites for) build
steps are identified by one of the following **target IDs**:
...
A **indirection** is an expression that takes the form `@VAR` or
`CLASS@VAR`.
...
`var` as above, `C1@C2@sources`, would yield `C1(C2(a.c)) C1(C2(b.c))`.
